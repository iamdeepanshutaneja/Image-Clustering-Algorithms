...
class image_clustering:
	...
	def __init__(self, folder_path="python/ouput/cluster0", n_clusters=10, max_examples=None, use_imagenets=False, use_pca=False):
		...
		os.makedirs("./pythonlab/output")
		for i in range(self.n_clusters):
			os.makedirs(".\\pythonlab\\output\\cluster" + str(i))
	...
	def clustering(self):
		...
		for i in range(self.max_examples):
			shutil.copy2(self.folder_path+"\\"+self.image_paths[i], "pythonlab\\output\\cluster"+str(predictions[i]))
...
if __name__ == "__main__":
	...
	number_of_clusters = 3 # cluster names will be 0 to number_of_clusters-1
